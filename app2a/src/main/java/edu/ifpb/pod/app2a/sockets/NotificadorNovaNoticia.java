...
public class NotificadorNovaNoticia implements NovaNoticiaListener {
    ...
    private List<NoticiaPersistivel> noticiasANotificar;
    ...
    public NotificadorNovaNoticia() {
        ...
        for (NoticiaPersistivel noticia : noticiasANotificar) {
            avisar(noticia);
    ...
    public void avisar(NoticiaPersistivel noticia) {
        try {
            byte[] noticiaXML = ConversorXML.objetoParaXml(NoticiaPersistivel.class, noticia);
